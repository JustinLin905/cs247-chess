...
void GraphicsObserver::notify() {
  for (int i = 0; i < 8; ++i) {
    for (int j = 0; j < 8; ++j) {
      ...
      if (isWhite) {
        _w->fillRectangle(100 * i, 100 * j, 100, 100, Xwindow::White);
      } else {
        _w->fillRectangle(100 * i, 100 * j, 100, 100, Xwindow::Black);
      }
      ...
      if (!isWhite) {
        ...
        int x =
            100 * i;  // X coordinate of the larger rectangle's top-left corner
        int y =
            100 * j;  // Y coordinate of the larger rectangle's top-left corner
      ...
      char color_indicator =
          _chess_board->getSquare(Position{i, j}).getPiece()->getColor() ==
              ...
              ? 'w'
              : 'b';
      _w->drawString(100 * i + 50, 100 * j + 50,
