...
void GraphicsObserver::notify() {
    ...
    for (int i = 1; i < 9; ++i) {
        ...
        _w->drawString(100 * i + 50, 50, std::string{char(64 + i)});
    }

    for (int i = 0; i < 8; ++i) {
        _w->fillRectangle(0, 100 * (i + 1), 100, 100, Xwindow::Green);
        _w->drawString(50, 100 * (i + 1) + 50, std::to_string(8 - i));
        for (int j = 0; j < 8; ++j) {
            ...
            if (!isWhite) {
                ...
                int x = 100 * (j + 1);  // X coordinate of the larger rectangle's top-left corner
                int y = 100 * (i + 1);  // Y coordinate of the larger rectangle's top-left corner
            ...
            _w->drawString(100 * (j + 1) + 50, 100 * (i + 1) + 50, std::string{color_indicator} + std::string{cur});
        }
    }
}